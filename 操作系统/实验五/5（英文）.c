...
int main() {
    ...
    // 输入进程数量
    printf("please enter the number of pid: ");
    scanf("%d", &n);

    // 初始化进程
    Process processes[n];
    for (int i = 0; i < n; i++) {
        printf("please enter the arrival and the burst of P%d: ", i + 1);
    ...
    // 输入时间片大小（用于 RR）
    printf("please enter the size of time for RR: ");
    scanf("%d", &quantum);
...
void calculateAndPrintMetrics(Process processes[], int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // 输出每个进程的结果
        printf("pid:P%d  turnaround:%d  weighted_turnaround:%.2f\n", processes[i].pid, turnaround, weighted_turnaround);
    }
